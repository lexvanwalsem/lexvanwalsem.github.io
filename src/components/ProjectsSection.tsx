...
const ProjectCard = ({ project, index }: { project: Project; index: number }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ borderColor: project.link ? "#36A9E1" : undefined }}
    >
      <Component 
        href={project.link} 
      ...
      >
        ...
        <div className="flex flex-wrap gap-2 mt-6">
          {project.tech.map((t) => (
            <span
              key={t}
              className="text-[10px] px-2 py-1 border border-border text-muted-foreground tracking-wider uppercase group-hover:border-primary/30 group-hover:text-primary/70 transition-colors font-mono"
            >
              {t}
            </span>
          ))}
    ...
    <section className="relative py-24 px-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-4 mb-12">
          <span className="text-[10px] text-muted-foreground tracking-widest font-mono">02</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 auto-rows-min">
